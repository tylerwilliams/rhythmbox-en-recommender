...
### THIS IS A HACKED VERSION TO SUPPORT ASYNC IO
"""
import rb
import urllib
import urllib2
...
#loader.get_url(url, get_successful_response)
def acallm(callback, method, param_dict, POST=False, socket_timeout=None, data=None):
    """
    Call the api! 
    ...
    ** note, if we require 2.6, we can get rid of this timeout munging.
    ...
    def finish(data):
        print "finish!"
        response_dict = get_successful_response(data)
        callback(response_dict)
    print "fetching url:",url
    loader.get_url(url, finish)
